...
def self.news_counter(search)
  agent = Mechanize.new
  page = agent.get"https://search.naver.com/search.naver?sm=tab_hty.top&where=news&ie=utf8&query=#{search}"
  number = page.search("div.title_desc_span").map(&:text).to_s
